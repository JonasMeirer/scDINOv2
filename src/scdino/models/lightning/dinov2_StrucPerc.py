"""Lightning module for the DINOv2 StrucPerc Perceiver backbone.

Mirrors ``src/scdino/models/lightning/dinov2.py``. Only the backbone import
and ``save_pretrained`` are different; the rest of the training/eval logic
is reused as-is, since ``MaskedPerceiverStrucPerc`` exposes the same public
surface as ``MaskedVisionTransformerTIMM``.
"""
...
from src.scdino.models.backbones.dinov2_StrucPerc import (
    DINOv2StrucPerc as DINOv2Skeleton,
)
...
class DINOv2StrucPerc(L.LightningModule):
    def __init__(
        self,
        name: str,
    ...
    def save_pretrained(self, save_directory: str, **kwargs) -> None:
        ...
        config = ScDINOConfig(
            ...
            in_chans=perc_cfg.get("in_chans", 5),
            img_size=perc_cfg.get("img_size", 50),
            ...
            num_self_blocks=perc_cfg.get("num_self_blocks", 8),
            num_cross_blocks=perc_cfg.get("num_cross_blocks", 1),
            depth_outer=perc_cfg.get("depth_outer", 1),
            rope_base=perc_cfg.get("rope_base", 100.0),
            rope_min_period=perc_cfg.get("rope_min_period", None),
            rope_max_period=perc_cfg.get("rope_max_period", None),
            rope_normalize_coords=perc_cfg.get("rope_normalize_coords", "separate"),
    ...
    def training_step(
        ...
        # Masking on the LATENT grid (latent_H * latent_W = num_patches).
        B = len(global_views)
